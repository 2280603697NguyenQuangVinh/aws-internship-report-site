...
The GreenLens project was developed by a team of four members. I served as the team leader and was responsible for the backend development, core business logic, AWS account management, AWS services configuration, and deployment of the system to AWS. Loi also contributed to the backend work, while Phuc and Hieu were responsible for frontend development.
...
- Backend development, business logic, AWS management, and deployment were assigned to me and Loi.
...
On the backend side, Loi and I developed the core logic of the project and implemented the APIs used by the frontend. This included handling business rules, processing requests, returning responses, and making sure the data flow between the client and server remained correct and reliable.

In addition to the application logic, I also managed the AWS account and configured the AWS services required by the project. This ensured the backend could be deployed and operated in a stable cloud environment.

## 6. AWS Setup and Deployment

After the main features were completed, the system was prepared for deployment on AWS. I was responsible for setting up the cloud environment, configuring the necessary services, and deploying the application so the website could run in a production-like environment.
